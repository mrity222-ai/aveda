...
export default function Header() {
  ...
  <Image
    ...
    width={42}
    height={42}
    className="rounded-sm hover:opacity-90 transition-opacity duration-200"
  />
  <span className="font-semibold text-lg text-white hover:text-primary transition">
    Aveda Technologies
  </span>
</Link>
        ...
        <nav className="hidden md:flex items-center gap-1 text-sm font-medium">
          {navLinks.map((link) => (
            <Link key={link.href} href={link.href} className="px-3 py-2 text-foreground/80 hover:text-[#e11d48] transition-colors duration-300">
              {link.label}
            </Link>
          ))}
        ...
        <div className="md:hidden">
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            ...
            <SheetContent side="right" className="w-full max-w-sm">
              <div className="grid gap-6 py-6">
                <Link href="/" className="flex items-center space-x-2 px-6" onClick={() => setIsOpen(false)}>
                  <Logo />
                </Link>
                <div className="flex flex-col gap-0 pl-6 pr-2">
                  {navLinks.map((link) => (
                  ...
                  <Accordion type="single" collapsible className="w-full">
                    <AccordionItem value="industries" className="border-b-0">
                      <AccordionTrigger className="flex w-full items-center py-3 text-lg font-semibold hover:no-underline">
                        Industries
                      </AccordionTrigger>
                      <AccordionContent className="overflow-y-auto max-h-[50vh]">
                        <div className="flex flex-col gap-2 pl-4 pr-2">
